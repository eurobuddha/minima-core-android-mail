 ...
 * module (the Mail app today, a native miniMerch/pocketShop tomorrow).
 *
...
public final class CommsIdentity {
    ...
    public static CommsIdentity fromSeed(LazySodium ls, byte[] minimaSeed) {
        byte[] boxSeed  = Hkdf.derive(minimaSeed, "minima-comms-box-v1",  Box.SEEDBYTES);
        byte[] signSeed = Hkdf.derive(minimaSeed, "minima-comms-sign-v1", Sign.SEEDBYTES);
